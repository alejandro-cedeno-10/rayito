"""`SandboxPool` de punta a punta sobre el plano de control falso (máquina
de estados + token buckets en un reloj falso) y un `rayd` falso por plaza."""

from __future__ import annotations

import json
import logging
import threading
import time
from collections.abc import Callable, Iterator
from datetime import UTC, datetime, timedelta
from pathlib import Path
from typing import Any

import pytest

from rayito import (
    IdlePolicy,
    InMemoryPoolBackend,
    JsonFilePoolBackend,
    PoolClosedException,
    PoolConfig,
    Sandbox,
    SandboxPool,
)
from rayito._payload import access_token_sha256
from rayito._pool_base import SlotRecord
from rayito._transport import ACCESS_TOKEN_KEY
from rayito.exceptions import (
    AuthenticationException,
    InvalidArgumentException,
    QuotaExceededException,
)

from .conftest import DEADLINE_KEY, IMAGE_ARN, JWE, FakeClock
from .fake_control_plane import FakeControlPlane, PoolTransport, max_calls_in_window
from .fake_process import presented_token_sha256

WAIT_BUDGET_SECONDS = 15.0
POLL_SECONDS = 0.02
POOL_TIMEOUT = 7200
POOL_MIN_REMAINING = 3600
TOKEN_LENGTH = 43


class TickingNow:
    """Un reloj de pared manual: cada lectura avanza `step` s y `advance`
    salta lo que haga falta (reciclado)."""

    def __init__(self, step: float = 0.0) -> None:
        self.current = datetime(2026, 9, 16, 12, 0, 0, tzinfo=UTC)
        self.step = step

    def __call__(self) -> datetime:
        value = self.current
        self.current = self.current + timedelta(seconds=self.step)
        return value

    def advance(self, seconds: float) -> None:
        self.current = self.current + timedelta(seconds=seconds)


class SpyBackend(InMemoryPoolBackend):
    """Anota cada estado guardado por plaza."""

    def __init__(self) -> None:
        super().__init__()
        self.saved: dict[str, list[str]] = {}

    def save(self, record: SlotRecord) -> None:
        self.saved.setdefault(record.sandbox_id, []).append(record.state)
        super().save(record)


class BrokenWarmingSaveBackend(SpyBackend):
    """Rechaza el primer `save` del estado `warming` y luego funciona."""

    def __init__(self) -> None:
        super().__init__()
        self.rejected = 0

    def save(self, record: SlotRecord) -> None:
        if record.state == "warming" and self.rejected == 0:
            self.rejected += 1
            raise InvalidArgumentException("backend roto")
        super().save(record)


@pytest.fixture
def now() -> TickingNow:
    return TickingNow()


@pytest.fixture
def clock() -> FakeClock:
    return FakeClock()


@pytest.fixture
def plane(clock: FakeClock, now: TickingNow) -> Iterator[FakeControlPlane]:
    fake = FakeControlPlane(clock=clock, now=now)
    try:
        yield fake
    finally:
        fake.close()


@pytest.fixture
def transport() -> PoolTransport:
    return PoolTransport.for_pool()


@pytest.fixture
def sleeps() -> list[float]:
    return []


PoolFactory = Callable[..., SandboxPool]


@pytest.fixture
def make_pool(
    plane: FakeControlPlane,
    transport: PoolTransport,
    clock: FakeClock,
    now: TickingNow,
    sleeps: list[float],
) -> Iterator[PoolFactory]:
    pools: list[SandboxPool] = []

    def factory(size: int = 2, **overrides: Any) -> SandboxPool:
        backend = overrides.pop("backend", None)
        config = PoolConfig(
            size=size,
            template=IMAGE_ARN,
            timeout=POOL_TIMEOUT,
            min_remaining_seconds=POOL_MIN_REMAINING,
            ready_timeout=10.0,
            **overrides,
        )
        pool = SandboxPool(
            config,
            backend=backend,
            control_plane=plane,
            transport=transport,
            monotonic=clock,
            now=now,
            sleep=sleeps.append,
            random=lambda: 0.5,
        )
        pools.append(pool)
        return pool

    try:
        yield factory
    finally:
        plane.release_all()
        for pool in pools:
            pool.close()


def wait_until(
    predicate: Callable[[], bool], what: str, budget: float = WAIT_BUDGET_SECONDS
) -> None:
    deadline = time.monotonic() + budget
    while not predicate():
        assert time.monotonic() < deadline, f"{what} no ocurrió en {budget:g} s"
        time.sleep(POLL_SECONDS)


def wait_idle(pool: SandboxPool, ready: int, what: str = "el pool no se llenó") -> None:
    wait_until(lambda: pool.stats().ready == ready and pool.stats().warming == 0, what)


def ready_ids(pool: SandboxPool) -> list[str]:
    return [slot.sandbox_id for slot in pool.stats().slots if slot.state == "ready"]


def ops_for(plane: FakeControlPlane, sandbox_id: str, *, since: int = 0) -> list[str]:
    return [call.operation for call in plane.calls[since:] if call.sandbox_id == sandbox_id]


# ------------------------------------------------------------------ warm-up


def test_warm_up_sequence_and_record_contents(
    make_pool: PoolFactory, plane: FakeControlPlane, transport: PoolTransport
) -> None:
    pool = make_pool(size=2).start()
    wait_idle(pool, 2)

    records = pool.backend.load()
    assert len(records) == 2
    assert {record.state for record in records} == {"ready"}
    assert all(record.parked_at is not None for record in records)
    assert len({record.access_token for record in records}) == 2
    for record in records:
        vm = plane.microvms[record.sandbox_id]
        assert vm.token_sha256 == access_token_sha256(record.access_token)
        assert vm.state == "SUSPENDED"
        assert ops_for(plane, record.sandbox_id) == [
            "CreateMicrovmAuthToken",
            "GetMicrovm",
            "SuspendMicrovm",
            "GetMicrovm",
        ]
        health = vm.rayd.servicer.health_calls
        assert health and health[0][ACCESS_TOKEN_KEY] == record.access_token
        assert record.endpoint == vm.endpoint
        assert record.idle == IdlePolicy(
            max_idle_seconds=300, suspended_duration_seconds=POOL_TIMEOUT - 300
        )
    assert len(plane.calls_to("RunMicrovm")) == 2
    assert transport.currently_open == 0
    assert transport.all_closed
    assert pool.stats().launched == 2


def test_record_saved_before_the_park(make_pool: PoolFactory, plane: FakeControlPlane) -> None:
    backend = SpyBackend()
    plane.fail("SuspendMicrovm", 1, RuntimeError("suspend exploded"))
    pool = make_pool(size=1, backend=backend).start()

    wait_until(lambda: pool.stats().failed == 1, "el calentamiento no falló")
    wait_idle(pool, 1)

    failed_id = next(vm.sandbox_id for vm in plane.microvms.values() if vm.state == "TERMINATED")
    assert backend.saved[failed_id] == ["warming"]
    assert failed_id not in {record.sandbox_id for record in backend.load()}
    assert "TerminateMicrovm" in ops_for(plane, failed_id)
    assert pool.stats().failed == 1
    assert pool.stats().launched == 2


def test_fill_respects_concurrency_and_bucket_pacing(
    make_pool: PoolFactory, plane: FakeControlPlane, transport: PoolTransport
) -> None:
    pool = make_pool(size=6, fill_concurrency=2).start()
    wait_idle(pool, 6)

    runs = plane.timestamps("RunMicrovm")
    suspends = plane.timestamps("SuspendMicrovm")
    assert len(runs) == 6 and len(suspends) == 6
    assert max_calls_in_window(runs, 1.0) <= 10
    assert max_calls_in_window(suspends, 1.0) <= 4
    assert max(suspends) - min(suspends) >= (6 - 2) / 2
    assert transport.max_open <= 2


# --------------------------------------------------------------------- take


def test_take_pops_the_oldest_and_resumes_explicitly(
    make_pool: PoolFactory, plane: FakeControlPlane, now: TickingNow
) -> None:
    now.step = 60.0
    pool = make_pool(size=2).start()
    wait_idle(pool, 2)
    records = {record.sandbox_id: record for record in pool.backend.load()}
    oldest = min(records.values(), key=lambda record: record.expires_at)
    before = len(plane.calls)

    sandbox = pool.take()
    try:
        assert sandbox.sandbox_id == oldest.sandbox_id
        assert sandbox.access_token == oldest.access_token
        assert ops_for(plane, oldest.sandbox_id, since=before) == [
            "ResumeMicrovm",
            "CreateMicrovmAuthToken",
        ]
        assert oldest.sandbox_id not in {record.sandbox_id for record in pool.backend.load()}
        assert sandbox.run_code("1+1").text == "2"
        assert sandbox.get_health().sandbox_id == oldest.sandbox_id
        sandbox.get_metrics()
        presented = plane.microvms[oldest.sandbox_id].rayd.servicer.metrics_calls[-1]
        assert presented_token_sha256(presented) == access_token_sha256(oldest.access_token)
        assert sandbox.info.state == "SUSPENDED"
        stats = pool.stats()
        assert stats.hits == 1 and stats.takes == 1 and stats.misses == 0
    finally:
        sandbox.kill()
    wait_idle(pool, 2, "el pool no rellenó tras la toma")
    assert pool.stats().launched == 3


def test_take_poll_schedule_is_the_fast_one(
    make_pool: PoolFactory, plane: FakeControlPlane, monkeypatch: pytest.MonkeyPatch
) -> None:
    seen: list[str] = []
    original = Sandbox._wait_until_ready

    def spy(self: Sandbox, **kwargs: Any) -> Any:
        seen.append(kwargs["readiness"].__name__)
        return original(self, **kwargs)

    monkeypatch.setattr(Sandbox, "_wait_until_ready", spy)
    pool = make_pool(size=1).start()
    wait_idle(pool, 1)

    pool.take().kill()

    assert seen[0] == "ReadinessPoll"
    assert "TakePoll" in seen


def test_empty_pool_falls_back_to_create(make_pool: PoolFactory, plane: FakeControlPlane) -> None:
    pool = make_pool(size=2).start()
    wait_idle(pool, 2)
    parked = set(ready_ids(pool))
    plane.hold("RunMicrovm", 2)

    first = pool.take()
    second = pool.take()
    wait_until(lambda: len(plane.calls_to("RunMicrovm")) == 4, "los rellenos no llegaron al run")
    third = pool.take()
    try:
        assert {first.sandbox_id, second.sandbox_id} == parked
        assert third.sandbox_id not in parked
        assert third.run_code("1+1").text == "2"
        stats = pool.stats()
        assert stats.hits == 2 and stats.misses == 1 and stats.takes == 3
        assert len(plane.calls_to("RunMicrovm")) == 5
    finally:
        plane.release("RunMicrovm")
        for sandbox in (first, second, third):
            sandbox.kill()
    wait_idle(pool, 2)
    assert pool.stats().launched == 5


def test_take_wait_is_served_by_a_refill(make_pool: PoolFactory, plane: FakeControlPlane) -> None:
    plane.hold("RunMicrovm", 1)
    pool = make_pool(size=1).start()
    wait_until(lambda: len(plane.calls_to("RunMicrovm")) == 1, "el calentamiento no arrancó")
    threading.Timer(0.3, plane.release, args=("RunMicrovm",)).start()

    started = time.monotonic()
    sandbox = pool.take(wait=5)
    try:
        assert time.monotonic() - started < 5
        assert sandbox.sandbox_id in plane.microvms
        stats = pool.stats()
        assert stats.hits == 1 and stats.misses == 0
    finally:
        sandbox.kill()


def test_take_without_wait_on_empty_pool_does_not_block(
    make_pool: PoolFactory, plane: FakeControlPlane
) -> None:
    plane.hold("RunMicrovm", 1)
    pool = make_pool(size=1).start()
    wait_until(lambda: len(plane.calls_to("RunMicrovm")) == 1, "el calentamiento no arrancó")

    sandbox = pool.take()
    try:
        assert pool.stats().misses == 1
    finally:
        plane.release("RunMicrovm")
        sandbox.kill()


def test_slot_lost_at_take_falls_back(make_pool: PoolFactory, plane: FakeControlPlane) -> None:
    pool = make_pool(size=1).start()
    wait_idle(pool, 1)
    lost_id = ready_ids(pool)[0]
    plane.fail("ResumeMicrovm", 1, False)

    sandbox = pool.take()
    try:
        assert sandbox.sandbox_id != lost_id
        assert sandbox.run_code("1+1").text == "2"
        assert "TerminateMicrovm" in ops_for(plane, lost_id)
        stats = pool.stats()
        assert (stats.lost, stats.misses, stats.hits, stats.takes) == (1, 1, 0, 1)
    finally:
        sandbox.kill()
    wait_idle(pool, 1)


def test_open_failure_at_take_falls_back(make_pool: PoolFactory, plane: FakeControlPlane) -> None:
    pool = make_pool(size=1).start()
    wait_idle(pool, 1)
    failing_id = ready_ids(pool)[0]
    plane.fail("CreateMicrovmAuthToken", 2, AuthenticationException("denied"))

    sandbox = pool.take()
    try:
        assert sandbox.sandbox_id != failing_id
        assert "TerminateMicrovm" in ops_for(plane, failing_id)
        stats = pool.stats()
        assert (stats.failed, stats.misses, stats.hits, stats.lost) == (1, 1, 0, 0)
    finally:
        sandbox.kill()


# ------------------------------------------------------------------- sweep


def test_recycle_with_a_fake_clock(
    make_pool: PoolFactory, plane: FakeControlPlane, now: TickingNow
) -> None:
    pool = make_pool(size=1).start()
    wait_idle(pool, 1)
    old_id = ready_ids(pool)[0]

    now.advance(3700)
    pool._request_sweep()

    wait_until(lambda: pool.stats().recycled == 1, "no se recicló")
    wait_idle(pool, 1)
    assert ready_ids(pool) != [old_id]
    assert "TerminateMicrovm" in ops_for(plane, old_id)
    assert plane.microvms[old_id].state == "TERMINATED"
    assert pool.stats().launched == 2


def test_reconcile_terminated_out_of_band(
    make_pool: PoolFactory, plane: FakeControlPlane, caplog: pytest.LogCaptureFixture
) -> None:
    pool = make_pool(size=1).start()
    wait_idle(pool, 1)
    dead_id = ready_ids(pool)[0]
    plane.set_state(dead_id, "TERMINATED", reason="Success.")
    gets_before = len(ops_for(plane, dead_id))

    with caplog.at_level(logging.WARNING, logger="rayito.pool"):
        pool._request_sweep()
        wait_until(lambda: pool.stats().lost == 1, "no se detectó la pérdida")
        wait_idle(pool, 1)

    assert ops_for(plane, dead_id)[gets_before:] == ["GetMicrovm"]
    assert ready_ids(pool) != [dead_id]
    assert any(
        dead_id in record.getMessage() and "Success." in record.getMessage()
        for record in caplog.records
    )


def test_reconcile_absent_and_not_found(make_pool: PoolFactory, plane: FakeControlPlane) -> None:
    pool = make_pool(size=1).start()
    wait_idle(pool, 1)
    gone_id = ready_ids(pool)[0]
    del plane.microvms[gone_id]

    pool._request_sweep()

    wait_until(lambda: pool.stats().lost == 1, "no se detectó la pérdida")
    wait_idle(pool, 1)
    assert gone_id not in ready_ids(pool)


def test_reconcile_resumed_out_of_band_is_reparked(
    make_pool: PoolFactory, plane: FakeControlPlane, caplog: pytest.LogCaptureFixture
) -> None:
    pool = make_pool(size=1).start()
    wait_idle(pool, 1)
    slot_id = ready_ids(pool)[0]
    plane.set_state(slot_id, "RUNNING")
    suspends_before = len(plane.calls_to("SuspendMicrovm"))

    with caplog.at_level(logging.WARNING, logger="rayito.pool"):
        pool._request_sweep()
        wait_until(
            lambda: len(plane.calls_to("SuspendMicrovm")) == suspends_before + 1,
            "no se volvió a aparcar",
        )

    assert plane.microvms[slot_id].state == "SUSPENDED"
    assert ready_ids(pool) == [slot_id]
    reparked = [r for r in caplog.records if "vuelta a aparcar" in r.getMessage()]
    assert len(reparked) == 1 and slot_id in reparked[0].getMessage()
    assert pool.stats().lost == 0


def test_listing_failure_aborts_the_sweep(
    make_pool: PoolFactory, plane: FakeControlPlane, caplog: pytest.LogCaptureFixture
) -> None:
    pool = make_pool(size=1).start()
    wait_idle(pool, 1)
    slot_id = ready_ids(pool)[0]
    plane.set_state(slot_id, "TERMINATED", reason="Success.")
    listings = len(plane.calls_to("ListMicrovms"))
    plane.fail("ListMicrovms", listings + 1, RuntimeError("throttled"))

    with caplog.at_level(logging.WARNING, logger="rayito.pool"):
        pool._request_sweep()
        wait_until(lambda: len(plane.calls_to("ListMicrovms")) == listings + 1, "sin barrido")
        time.sleep(0.1)

    assert ready_ids(pool) == [slot_id]
    assert any("list-microvms" in r.getMessage() for r in caplog.records)


# ----------------------------------------------------------------- backoff


def test_backoff_after_failures(
    make_pool: PoolFactory,
    plane: FakeControlPlane,
    sleeps: list[float],
    caplog: pytest.LogCaptureFixture,
) -> None:
    plane.fail("RunMicrovm", 1, QuotaExceededException("memory quota"))
    plane.fail("RunMicrovm", 2, QuotaExceededException("memory quota"))

    with caplog.at_level(logging.WARNING, logger="rayito.pool"):
        pool = make_pool(size=1).start()
        wait_idle(pool, 1)

    assert sleeps == [1.0, 2.0]
    assert len(plane.calls_to("RunMicrovm")) == 3
    assert pool.stats().failed == 2
    warnings = [r.getMessage() for r in caplog.records if r.levelno == logging.WARNING]
    assert len(warnings) == 2
    assert all("QuotaExceededException" in message for message in warnings)
    token = pool.backend.load()[0].access_token
    assert not any(token in message for message in warnings)


def test_backend_failure_at_launch_terminates_the_vm(
    make_pool: PoolFactory, plane: FakeControlPlane, sleeps: list[float]
) -> None:
    backend = BrokenWarmingSaveBackend()
    pool = make_pool(size=1, backend=backend).start()
    wait_idle(pool, 1)

    launched = list(plane.microvms)
    assert len(launched) == 2
    assert plane.microvms[launched[0]].state == "TERMINATED"
    assert ops_for(plane, launched[0]) == ["TerminateMicrovm"]
    assert backend.rejected == 1
    assert pool.stats().failed == 1
    assert pool.stats().launched == 1
    assert sleeps == [1.0]
    assert plane.live_ids == ready_ids(pool) == [launched[1]]
    assert [record.sandbox_id for record in backend.load()] == [launched[1]]


def test_backoff_resets_after_success(
    make_pool: PoolFactory, plane: FakeControlPlane, sleeps: list[float]
) -> None:
    plane.fail("RunMicrovm", 1, QuotaExceededException("q"))
    plane.fail("RunMicrovm", 3, QuotaExceededException("q"))
    pool = make_pool(size=1).start()
    wait_idle(pool, 1)

    pool.take().kill()
    wait_idle(pool, 1)

    assert sleeps == [1.0, 1.0]


# ------------------------------------------------------------------- close


def test_close_drains_and_joins(make_pool: PoolFactory, plane: FakeControlPlane) -> None:
    with make_pool(size=3) as pool:
        wait_idle(pool, 3)
        ids = ready_ids(pool)
        filler = pool._filler

    assert filler is not None and not filler.is_alive()
    for sandbox_id in ids:
        assert "TerminateMicrovm" in ops_for(plane, sandbox_id)
        assert plane.microvms[sandbox_id].state == "TERMINATED"
    assert pool.backend.load() == ()
    assert plane.live_ids == []
    with pytest.raises(PoolClosedException):
        pool.take()
    pool.close()


def test_close_with_warm_up_in_flight_terminates_instead_of_parking(
    make_pool: PoolFactory, plane: FakeControlPlane
) -> None:
    plane.hold("RunMicrovm", 1)
    pool = make_pool(size=1).start()
    wait_until(lambda: len(plane.calls_to("RunMicrovm")) == 1, "el calentamiento no arrancó")

    closer = threading.Thread(target=pool.close)
    closer.start()
    time.sleep(0.1)
    plane.release("RunMicrovm")
    closer.join(timeout=WAIT_BUDGET_SECONDS)

    assert not closer.is_alive()
    assert plane.live_ids == []
    assert pool.backend.load() == ()


def test_no_drain_needs_persistence(
    make_pool: PoolFactory, plane: FakeControlPlane, tmp_path: Path
) -> None:
    memory = make_pool(size=1).start()
    wait_idle(memory, 1)
    with pytest.raises(InvalidArgumentException, match=r"drain.*persistent"):
        memory.close(drain=False)
    memory.close()

    backend = JsonFilePoolBackend(tmp_path / "pool.json")
    persistent = make_pool(size=1, backend=backend).start()
    wait_idle(persistent, 1)
    slot_id = ready_ids(persistent)[0]
    terminates = len(plane.calls_to("TerminateMicrovm"))

    persistent.close(drain=False)

    assert [record.sandbox_id for record in backend.load()] == [slot_id]
    assert len(plane.calls_to("TerminateMicrovm")) == terminates
    assert plane.microvms[slot_id].state == "SUSPENDED"


def test_recovery_after_a_simulated_crash(
    make_pool: PoolFactory, plane: FakeControlPlane, tmp_path: Path
) -> None:
    path = tmp_path / "pool.json"
    first = make_pool(size=2, backend=JsonFilePoolBackend(path)).start()
    wait_idle(first, 2)
    first.close(drain=False)
    orphan_id, ready_id = sorted(ready_ids(first))
    document = json.loads(path.read_text(encoding="utf-8"))
    for slot in document["slots"]:
        if slot["sandbox_id"] == orphan_id:
            slot["state"] = "warming"
            slot["parked_at"] = None
    path.write_text(json.dumps(document), encoding="utf-8")
    terminates = len(plane.calls_to("TerminateMicrovm"))

    second = make_pool(size=1, backend=JsonFilePoolBackend(path)).start()

    assert plane.calls_to("TerminateMicrovm")[terminates:][0].sandbox_id == orphan_id
    assert len(plane.calls_to("TerminateMicrovm")) == terminates + 1
    stats = second.stats()
    assert (stats.ready, stats.lost, stats.launched) == (1, 1, 0)
    sandbox = second.take()
    try:
        assert sandbox.sandbox_id == ready_id
        assert sandbox.run_code("1+1").text == "2"
    finally:
        sandbox.kill()


# ------------------------------------------------------------ create(pool=)


def test_create_with_pool_is_sugar_for_take(make_pool: PoolFactory) -> None:
    pool = make_pool(size=1).start()
    wait_idle(pool, 1)

    sandbox = Sandbox.create(pool=pool)
    try:
        assert pool.stats().hits == 1
        assert sandbox.run_code("1+1").text == "2"
    finally:
        sandbox.kill()
    with pytest.raises(InvalidArgumentException, match="`envs`"):
        Sandbox.create(pool=pool, envs={"A": "1"})


@pytest.mark.parametrize(
    ("name", "value"),
    [
        ("template", IMAGE_ARN),
        ("template_version", "2.0"),
        ("timeout", 900),
        ("idle", None),
        ("envs", {"A": "1"}),
        ("metadata", {"a": "b"}),
        ("cpu_time_limit", 10),
        ("execution_role_arn", "arn:aws:iam::1:role/r"),
        ("allowed_ports", [3000]),
        ("ingress", ["ALL_INGRESS"]),
        ("egress", ["INTERNET_EGRESS"]),
        ("logging", "cloudwatch"),
        ("region", "us-east-1"),
        ("session", object()),
        ("access_token", "a" * TOKEN_LENGTH),
        ("keep_on_failure", True),
        ("control_plane", object()),
        ("transport", object()),
    ],
)
def test_create_with_pool_rejects_each_launch_kwarg(
    make_pool: PoolFactory, name: str, value: object
) -> None:
    pool = make_pool(size=1)
    kwargs: Any = {name: value}

    with pytest.raises(InvalidArgumentException, match=f"`{name}`"):
        Sandbox.create(pool=pool, **kwargs)


def test_create_with_pool_passes_the_knobs_through(
    make_pool: PoolFactory, plane: FakeControlPlane
) -> None:
    pool = make_pool(size=1).start()
    wait_idle(pool, 1)

    sandbox = Sandbox.create(
        pool=pool, request_timeout=7.5, ready_timeout=8.0, reconnect_timeout=9.0
    )
    try:
        sandbox.get_metrics()
        seen = plane.microvms[sandbox.sandbox_id].rayd.servicer.metrics_calls[-1][DEADLINE_KEY]
        assert 5.0 < float(seen) < 8.0
        assert sandbox._ready_timeout == 8.0
        assert sandbox._reconnect_timeout == 9.0
    finally:
        sandbox.kill()


def test_create_with_unstarted_pool_raises_pool_closed(make_pool: PoolFactory) -> None:
    pool = make_pool(size=1)

    with pytest.raises(PoolClosedException, match="not started"):
        Sandbox.create(pool=pool)
    with pytest.raises(PoolClosedException):
        pool.take()


# ----------------------------------------------------------------- secrets


def test_caplog_holds_no_secret(
    make_pool: PoolFactory, plane: FakeControlPlane, caplog: pytest.LogCaptureFixture
) -> None:
    with caplog.at_level(logging.DEBUG):
        pool = make_pool(size=2).start()
        wait_idle(pool, 2)
        tokens = [record.access_token for record in pool.backend.load()]
        sandbox = pool.take()
        tokens.append(sandbox.access_token)
        sandbox.kill()
        wait_idle(pool, 2)
        lost_id = ready_ids(pool)[0]
        plane.set_state(lost_id, "TERMINATED", reason="Success.")
        pool._request_sweep()
        wait_until(lambda: pool.stats().lost == 1, "sin pérdida")
        wait_idle(pool, 2)
        tokens.extend(record.access_token for record in pool.backend.load())
        pool.close()

    text = "\n".join(record.getMessage() for record in caplog.records)
    assert caplog.records
    assert JWE not in text
    assert "token_sha256" not in text
    assert all(token not in text for token in tokens)


# ------------------------------------------------------------------- stats


def test_stats_after_a_scripted_sequence(
    make_pool: PoolFactory, plane: FakeControlPlane, now: TickingNow
) -> None:
    now.step = 60.0
    pool = make_pool(size=2).start()
    wait_idle(pool, 2)
    now.step = 0.0
    oldest, newest = sorted(pool.backend.load(), key=lambda record: record.expires_at)
    assert oldest.expires_at < newest.expires_at
    now.current = newest.expires_at - timedelta(seconds=POOL_MIN_REMAINING)
    pool._request_sweep()
    wait_until(lambda: pool.stats().recycled == 1, "sin reciclado")
    wait_idle(pool, 2)
    plane.hold("RunMicrovm", 2)

    hit = pool.take()
    wait_until(lambda: len(plane.calls_to("RunMicrovm")) == 4, "el relleno no llegó al run")
    remaining_id = ready_ids(pool)[0]
    plane.set_state(remaining_id, "TERMINATED", reason="Success.")
    pool._request_sweep()
    wait_until(lambda: len(plane.calls_to("RunMicrovm")) == 5, "el segundo relleno no llegó")
    miss = pool.take()
    plane.release("RunMicrovm")
    wait_idle(pool, 2)
    hit.kill()
    miss.kill()

    stats = pool.stats()
    assert (stats.size, stats.ready, stats.warming) == (2, 2, 0)
    assert (stats.takes, stats.hits, stats.misses) == (2, 1, 1)
    assert (stats.launched, stats.recycled, stats.lost, stats.failed) == (6, 1, 1, 0)
    assert len(stats.slots) == 2
    assert all(slot.state == "ready" for slot in stats.slots)
    assert not any(hasattr(slot, "access_token") for slot in stats.slots)
