...
`rayito.cli._compat.COMPATIBILITY` SHALL be a tuple of `CompatibilityRow(sdk_series, min_agent_version, min_image_version, note)` starting with `("0.1", "0.1.0", "10.0", …)`, and `assess(sdk_version, agent_version, image_version)` SHALL return `FAIL` when the agent is below the row's minimum or the image below `min_image_version`, `WARN` when the agent's `MAJOR.MINOR` is newer than the SDK's, and `OK` otherwise, parsing versions as integer `MAJOR.MINOR.PATCH` (pre-release suffix ignored) and image versions numerically. `docs/site/docs/limits.md` SHALL contain a section `## Compatibilidad SDK ↔ rayd ↔ imagen` whose table rows equal `COMPATIBILITY`, and `tests/unit/cli/test_compat.py` SHALL parse that table and assert equality (skipped when the docs file is absent).

#### Scenario: assessments
- **WHEN** `assess("0.1.0", "0.1.0", "17.0")`, `assess("0.1.0", "0.0.9", "17.0")`, `assess("0.1.0", "0.2.0", "17.0")` and `assess("0.1.0", "0.1.0", "9.0")` are evaluated
- **THEN** the statuses are `OK`, `FAIL`, `WARN`, `FAIL` respectively, each with a reason naming the offending value
